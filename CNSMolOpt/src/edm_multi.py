...
class EDM(torch.nn.Module):
    ...
    def forward(self, x, h, x_batch_new, h_batch_new, node_mask_batch_new, scaffold_mask_batch_new, 
                ...
                edge_mask, context, center_of_mass_mask, batch_new_len_tensor, affinity_label_batch_new):
        #print(f"context shape: {node_mask_batch_new.shape}")
        ...
        # Sample t
        t_int = torch.randint(0, self.T + 1, size=(x.size(0), 1), device=x.device).float()#每个样本[0, self.T]随机采样一个时间步形状[batch_size, 1]
        #print(f'{t_int.shape}')
        #print(t_int)
        s_int = t_int - 1
        t = t_int / self.T#归一化
        ...
        #print(h_batch_new[0])
        #print(x_batch_new[0])
        x_batch_new, h_batch_new = self.normalize(x_batch_new, h_batch_new)#归一化
        #print(h_batch_new[0])
        #print(x_batch_new[0])
        xh_batch_new = torch.cat([x_batch_new, h_batch_new], dim=2)#节点类型和位置信息
        #print(f'{xh_batch_new.shape}')
        #print(xh_batch_new[0])
        ...
        z_t = xh_batch_new * scaffold_mask_ori_batch_new + z_t * rgroup_mask_ori_batch_new#真实噪声
        #print(f'z_t shape:{z_t.shape}')
        
        #print(context)
        #exit()
        # Neural net prediction
        #print(f"t_batch_new: {t_batch_new.shape}")
        ...
        #print(f'eps_t_hat shape:{eps_t_hat.shape}')
        #exit()
    ...
    @torch.no_grad()
    def sample_chain(self, x, h, node_mask, scaffold_mask, rgroup_mask, edge_mask, context, affinity_label_batch_new, keep_frames=None):
        n_samples = x.size(0)
        n_nodes = x.size(1)
        ...
        z = xh * scaffold_mask + z * rgroup_mask#初始噪声
    ...
    def sample_p_zs_given_zt_only_rgroup(self, s, t, z_t, node_mask, scaffold_mask, rgroup_mask, edge_mask, context, affinity_label_batch_new):
    ...
    def sample_p_xh_given_z0_only_rgroup(self, z_0, node_mask, scaffold_mask, rgroup_mask, edge_mask, context, affinity_label_batch_new):
