...
class EquivariantBlock(nn.Module):
    ...
    def forward(self, h, x, edge_index, node_mask=None, rgroup_mask=None, edge_mask=None, edge_attr=None):
        ...
        for i in range(0, self.n_layers):
            h, _ = self._modules["gcl_%d" % i](h, edge_index, edge_attr=edge_attr, node_mask=node_mask, edge_mask=edge_mask)#gcl
...
class EGNN(nn.Module):
    ...
    def forward(self, h, x, edge_index, node_mask=None, rgroup_mask=None, edge_mask=None):
        # Edit Emiel: Remove velocity as input
        distances, _ = coord2diff(x, edge_index)#这里是什么,计算节点之间的距离，确定哪些节点是相邻的
        if self.sin_embedding is not None:
            distances = self.sin_embedding(distances)

        h = self.embedding(h)#使用全连接层输入
        for i in range(0, self.n_layers):
            h, x = self._modules["e_block_%d" % i](
                ...
                edge_mask=edge_mask,
                edge_attr=distances
            )#对每一层都调用对应的EquivariantBlock

        # Important, the bias of the last linear might be non-zero
        h = self.embedding_out(h)#经过全连接层处理后
        if node_mask is not None:
            h = h * node_mask
        return h, x
...
class Dynamics(nn.Module):#用这个预测
    def __init__(
    ...
    def forward(self, t, xh, node_mask, rgroup_mask, edge_mask, context):#运行这里
        """
        - t: (B)
...
class DynamicsWithPockets(Dynamics):
    def forward(self, t, xh, node_mask, rgroup_mask, edge_mask, context, affinity_label_batch_new):
        ...
        """
        #print(context[0])
        #print(f" {edge_mask.shape}")
        #exit()
        bs, n_nodes = xh.shape[0], xh.shape[1]#(B,N,15)B是批次，N是数据原子数，15是坐标长宽高和12个原子类型。
        node_mask = node_mask.view(bs * n_nodes, 1)  # (B*N, 1)这是什么，将批次中的所有节点展开？展成一列

        if rgroup_mask is not None:
            rgroup_mask = rgroup_mask.view(bs * n_nodes, 1)  # (B*N, 1)

        # Reshaping node features & adding time feature
        xh = xh.view(bs * n_nodes, -1).clone() * node_mask  # (B*N, D)，数据原子
        #print(f"context shape: {xh.shape}")
        #exit()
        x = xh[:, :self.n_dims].clone()  # (B*N, 3)坐标，前self.n_dims列
        h = xh[:, self.n_dims:].clone()  # (B*N, nf)类型，从self.n_dims到后几列
        #print(f"context shape: {h.shape}")
        #exit()
        edges = self.get_dist_edges(x, node_mask, edge_mask)
        print(f" {affinity_label_batch_new.shape}")
        #print(edges)
        
        if self.condition_time:#时间
            if np.prod(t.size()) == 1:
                # t is the same for all elements in batch.
                h_time = torch.empty_like(h[:, 0:1]).fill_(t.item())
