...
#class AdjustAffinityLabel(nn.Module):
    #def __init__(self, input_dim, context_dim):
        #super(AdjustAffinityLabel, self).__init__()
        # 全连接层，输入维度为 input_dim，输出维度为 context_dim
        #self.fc = nn.Linear(input_dim, context_dim)
    ...
    #def forward(self, x, context_size, device):
        ...
        #x = x.mean(dim=1)  
        #x = self.fc(x) 
        #x = x.unsqueeze(-1)  # [batch_size, num_nodes, 1]
        #x = x.expand(-1, num_nodes, feature_dim)  # [batch_size, num_nodes, feature_dim]
...
class DDPM(pl.LightningModule):
    ...
    def forward(self, data, training):
        ...
        #print(affinity_label_batch_new[0])
        #print(scaffold_mask_batch_new[0])
        #print(node_mask_batch_new[0])
        #exit()

        #affinity_label_batch_new = affinity_label_batch_new.unsqueeze(-1)
        

        #print(f"affinity_label shape: {affinity_label_batch_new.shape}")
        ...
        #print(f"context shape: {scaffold_only_mask.shape}")
        #print(f"context shape: {scaffold_mask_batch_new.shape}")
        ...
        # 调整 affinity_label
        #adjust_affinity = AdjustAffinityLabel(affinity_label_batch_new.size(1), output_dim=num_nodes).to(context.device)
        #affinity_label_adjusted = adjust_affinity(affinity_label_batch_new, context_size=(batch_size, num_nodes, feature_dim), device=context.device)
        #print(f"affinity_label_adjusted: {affinity_label_adjusted.shape}")
        #print(f"context: {context.shape}")
        #print(affinity_label_adjusted[0])
        #exit()
        ...
        #adjust_affinity = AdjustAffinityLabel(input_dim=4, output_dim=N).to(context.device)
        #affinity_label_batch_new = affinity_label_batch_new.to(context.device)
        #affinity_label_adjusted = adjust_affinity(affinity_label_batch_new, N, context.device)
        
        
        #print(f"affinity_label shape: {affinity_label_adjusted.shape}") 
    ...
    def sample_chain(self, data, sample_fn=None, keep_frames=None, target_affinity=None):
        ...
        #print(f"affinity_label_batch_new shape:{affinity_label_batch_new.shape}")
        ...
        #print(affinity_label_batch_newc)
        #print(f"affinity_label_batch_newc shape:{affinity_label_batch_newc.shape}")
        #exit()
        #affinity_one_hot[:, -1, :] = torch.eye(affinity_label_batch_new.size(-1))[-1]
        #affinity_label_batch_new = affinity_one_hot
                #print(affinity_label_batch_new[0])
        ...
        # 修改 affinity_label
            #if target_affinity is not None:
        # 强制将 affinity_label 设置为目标高值
            #affinity_label = torch.full_like(template_data['affinity_label_batch_new'], target_affinity)
            #else:
                #affinity_label = template_data['affinity_label_batch_new']  # 保留原始值
        ...
        # 调整 affinity_label
        #adjust_affinity = AdjustAffinityLabel(input_dim=affinity_label_batch_new.size(-1), context_dim=num_nodes).to(context.device)
        #affinity_label_adjusted = adjust_affinity(affinity_label_batch_new, (batch_size, num_nodes, feature_dim), context.device)
              #print(affinity_label_adjusted[0])
              #print(f"affinity_label shape: {affinity_label_adjusted.shape}") 
              # 将 adjusted affinity_label 添加到 context
        #context = torch.cat([context, affinity_label_adjusted], dim=-1)
              #print(f"context shape: {context.shape}") 
